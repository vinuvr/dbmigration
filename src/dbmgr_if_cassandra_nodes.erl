...
%%%-------------------------------------------------------------------
-module(dbmgr_if_cassandra_nodes).
...
add(CSNode) ->
  gen_server:call(?SERVER, {add, dbmgr_api_utils:to(binary, CSNode)}).

remove(CSNode) ->
  gen_server:call(?SERVER, {remove, dbmgr_api_utils:to(binary, CSNode)}).
