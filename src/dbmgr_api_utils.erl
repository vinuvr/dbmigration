...
-export([to/2,
         proplist_to_map/1,
         cassandra_put_query/2,
         cassandra_update_query/2,
...
key_tmap(Map) ->
  maps:fold(fun(K, V, Ac) ->
                Ac#{dbmgr_api_utils:to(atom, K) => V} end
            ,#{}, Map).
...
get_response_body(Out) ->
  try
    OutU = dbmgr_api_utils:to(binary, Out),
    jsx:decode(OutU, [return_maps])
  catch
    _:_ ->
      dbmgr_api_utils:to(binary, Out)
  end.
...
frame_metadata(#{method := Method, operation_id := OperationId, tag := Tag
  ...
  TagString = dbmgr_api_utils:to(list, Tag),
  TagBin = dbmgr_api_utils:to(binary, Tag),
  #{dbmgr_api_utils:to(atom, Method) =>
    #{tags => [TagString],
      description => << "Retrives ",  TagBin/binary , " information" >>,
      summary => Summary,
      operationId => dbmgr_api_utils:to(binary, OperationId),
...
frame_meta_params(Params) ->
  lists:foldl(
    fun(Name, Ac) ->
        NameU = dbmgr_api_utils:to(binary, Name),
        ...
        TokenU ->
          ...
          case keycloak_jwt:decode(TokenU) of
            #{<<"sub">> := UserId, <<"azp">> := _ClientN,
              <<"email">> := Email} ->
              case dbmgr_api_users:get_users(#{keycloak_id => UserId, email => Email}) of
                [] ->
            ...
            #{<<"sub">> := _, <<"azp">> := ClientN,
              ...
              case maps:get(<<"user-uid">>,maps:get(headers, Req, #{}),undefined) of
                undefined ->
                  error;
                UserId ->
                  case dbmgr_api_users:get_users(#{uuid => UserId}) of
                    [] ->
                      lager:info("Error 404"),
                      undefined;
...
%       case dbmgr_api_tenants:get_tenant_clients(#{name => ClientN}) of
%         [] ->
%           lager:info("Tenant Not Found"),
%           undefined;
%         [#{tenant := TenantUuid}] ->
%           lager:info("Required Params Sucess"),
%           case dbmgr_api_users:get_users(#{keycloak_id => UserId, email => Email, tenant_id => TenantUuid}) of
%             [] ->
% 							lager:info("Error 404"),
%               undefined;
%             [#{uuid := UserUuid}] ->
...
hrm_api(AccessToken, ClientName, UrlU) ->
  Url = dbmgr_api_configs:get_config(hrm, <<"https://dev.hrmnest.com/v3/tenant/">>),
  ...
  case dbmgr_api_utils:send_http_request(get, NewRequest) of
    error ->
