...
%%%-------------------------------------------------------------------
-module(dbmgr_if_cassandra_node).
...
init([CSNode]) ->
  ...
  start_ping_timer(),
  {ok, #{cs_node => dbmgr_api_utils:to(atom, server_name(CSNode)), connection => down
         ,client_info => CSNode}}.
...
server_name([{IP, Port}, _Spec] = Info) ->
  ...
  CSNodeBin = dbmgr_api_utils:to(
                binary, dbmgr_api_utils:to(list, IP) ++ ":" ++
                dbmgr_api_utils:to(list, Port)),
  dbmgr_api_utils:to(atom, <<"cs_node_", CSNodeBin/binary >>);
server_name(Other) ->
