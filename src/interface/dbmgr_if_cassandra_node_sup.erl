...
%%%-------------------------------------------------------------------
-module(dbmgr_if_cassandra_node_sup).
...
server_name([{IP, Port}, _Spec]) ->
  list_to_atom(dbmgr_api_utils:to(list, ?MODULE) ++ "_" ++ dbmgr_api_utils:to(list, IP)
               ++ ":" ++ dbmgr_api_utils:to(list, Port)).
...
init([CSNode]) ->
  SupervisorFlags = #{strategy => one_for_one,
                      ...
                      period => 60},
  ChildSpecs = [
                ?CHILD(dbmgr_if_cassandra_node, CSNode)
               % ?CHILD(dbmgr_if_fs_cassandra, CSNode),
               % ?CHILD(dbmgr_if_cassandra_out, CSNode)
               ],
